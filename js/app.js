...
function startTrapScanner() {
  ...
  function renderScanResultCard(scanResult, customSymbol, customTimestamp) {
    ...
    if (customSymbol) {
      ...
      symbol = activeConfig ? activeConfig.symbol : 'UNKNOWN';
      decimals = activeConfig ? activeConfig.decimals : 2;
    }
  ...
  // Seed historical data on initialization
  const seedResults = [
    {
      symbol: "BTC_USDT",
  ...
  seedResults.forEach(seed => {
    renderScanResultCard(seed.card, seed.symbol, seed.timestamp);
  });
  ...
  setInterval(() => {
    ...
    renderScanResultCard(scanResult, null, new Date().toLocaleTimeString());
  }, 5000);
}
...
function detectPatternAndTraps(activeConfig) {
  ...
  function checkCooldown(patternKey, cooldownMs) {
  ...
  // 1. Check for Stop Hunt Sweep
  if (volume > 28.0 && checkCooldown('trapStopHunt', 30000)) {
  ...
  // 2. Check for Bull Trap:
  if (localHighs.length > 0) {
    const recentHigh = localHighs[localHighs.length - 1];
    if (prevPrice2 > recentHigh.price && currentPrice < recentHigh.price && volume > 25.5 && checkCooldown('trapBullTrap', 30000)) {
  ...
  // 3. Check for Bear Trap:
  if (localLows.length > 0) {
    const recentLow = localLows[localLows.length - 1];
    if (prevPrice2 < recentLow.price && currentPrice > recentLow.price && volume > 25.5 && checkCooldown('trapBearTrap', 30000)) {
  ...
  if (lastDiff > avgDiff * 2.5 && checkCooldown('trapImbalance', 30000)) {
  ...
  // 5. Check for Double Bottom (Structural):
  if (localLows.length >= 2) {
    const low1 = localLows[localLows.length - 2].price;
    const low2 = localLows[localLows.length - 1].price;
    if (Math.abs(low1 - low2) / low1 < 0.0020 && rsi < 35 && checkCooldown('patternDoubleBottom', 40000)) {
  ...
  // 6. Check for Double Top / Head & Shoulders (Structural):
  if (localHighs.length >= 2) {
    const high1 = localHighs[localHighs.length - 2].price;
    const high2 = localHighs[localHighs.length - 1].price;
    if (Math.abs(high1 - high2) / high1 < 0.0020 && rsi > 65 && checkCooldown('patternHeadShoulders', 40000)) {
  ...
  if (rsi > 70) {
    if (!state.wasRsiOverbought) {
      ...
      if (checkCooldown('patternHeadShoulders', 40000)) {
        triggerPattern('patternHeadShoulders');
        return {
          type: 'PATTERN',
          patternKey: 'patternHeadShoulders',
  ...
  if (rsi < 30) {
    if (!state.wasRsiOversold) {
      ...
      if (checkCooldown('patternDoubleBottom', 40000)) {
        triggerPattern('patternDoubleBottom');
        return {
          type: 'PATTERN',
          patternKey: 'patternDoubleBottom',
